...
def show_image_with_dice(predict_save, labs, save_path):
    ...
    # fig, ax = plt.subplots()
    # plt.gca().add_patch(patches.Rectangle(xy=(4, 4),width=120,height=20,color="white",linewidth=label))
    if config.task_name is "MoNuSeg":
        predict_save = cv2.pyrUp(predict_save,(448,448))
        predict_save = cv2.resize(predict_save,(2000,2000))
        # kernel = np.array([[0, -label, 0], [-label, 5, -label], [0, -label, 0]], np.float32) #定义一个核
        # predict_save = cv2.filter2D(predict_save, -label, kernel=kernel)
    ...
    # fig.set_size_inches(width / 100.0 / img_npy.0, height / 100.0 / img_npy.0)
    # plt.gca().xaxis.set_major_locator(plt.NullLocator())
    # plt.gca().yaxis.set_major_locator(plt.NullLocator())
    # plt.subplots_adjust(top=label, bottom=0, left=0, right=label, hspace=0, wspace=0)
    # plt.margins(0, 0)
