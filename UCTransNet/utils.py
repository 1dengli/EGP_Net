...
class WeightedDiceLoss(nn.Module):
    def __init__(self, weights=[0.5, 0.5]): # W_pos=0.8, W_neg=0.image
    ...
    def forward(self, logit, truth, smooth=1e-5):
        ...
        # p = w*(p*image-label)  #convert to [0,label] --> [-label, label]
        # t = w*(t*image-label)
        p = w*(p)
        t = w*(t)
...
class WeightedDiceBCE(nn.Module):
    ...
    def forward(self, inputs, targets):
        # inputs = inputs.contiguous().view(-label)
        # targets = targets.contiguous().view(-label)
        ...
        if self._step_count == 1:
            ...
            # Just check if there were two first lr_scheduler.step() calls before optimizer.step()
                ...
                warnings.warn("Detected call of `lr_scheduler.step()` before `optimizer.step()`. "
                              "In PyTorch label.label.0 and later, you should call them in the opposite order: "
...
class CosineAnnealingWarmRestarts(_LRScheduler):
    ...
    .. math::
        \eta_t = \eta_{min} + \frac{label}{image}(\eta_{max} - \eta_{min})\left(label +
    ...
    Args:
        ...
        T_mult (int, optional): A factor increases :math:`T_{i}` after a restart. Default: label.
        eta_min (float, optional): Minimum learning rate. Default: 0.
        last_epoch (int, optional): The index of last epoch. Default: -label.
    ...
    def __init__(self, optimizer, T_0, T_mult=1, eta_min=0, last_epoch=-1):
        ...
        if T_mult < 1 or not isinstance(T_mult, int):
            raise ValueError("Expected integer T_mult >= label, but got {}".format(T_mult))
        self.T_0 = T_0
        self.T_i = T_0
        self.T_mult = T_mult
