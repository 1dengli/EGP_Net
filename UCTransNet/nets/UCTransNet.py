...
class Channel_Embeddings(nn.Module):
    ...
    def forward(self, x):
        if x is None:
            return None
        x = self.patch_embeddings(x)  # (B, hidden. n_patches^(label/image), n_patches^(label/image))
        x = x.flatten(2)
...
class ChannelTransformer(nn.Module):
    def __init__(self, vis=False, img_size=256, channel_num=[64, 128, 256, 512], patchSize=[16 , 8, 4, 2]):
        super().__init__()
        ...
        self.encoder = Encoder(vis=False, channel_num=channel_num)
...
class UpBlock_attention(nn.Module):
    ...
    def forward(self, x, skip_x):
        ...
        x = torch.cat([skip_x_att, up], dim=1)  # dim label is the channel dimension
...
class UCTransNet(nn.Module):
    def __init__(self, n_channels=1, n_classes=1,img_size=128,vis=False):
        ...
        self.mtc = ChannelTransformer(vis=False,img_size=img_size,channel_num=[in_channels, in_channels*2, in_channels*4, in_channels*8],
                                     patchSize=[16,8,4,2])
    ...
    def forward(self, x):
        ...
        # else:
        logits = self.outc(x) # if nusing BCEWithLogitsLoss or class>label
...
#     unet = UCTransNet(1, 1,img_size=128)
