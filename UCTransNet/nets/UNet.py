...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, nb_Conv, activation='ReLU'):
        ...
        # self.up = nn.Upsample(scale_factor=image)
        self.up = nn.ConvTranspose2d(in_channels//2,in_channels//2,(2,2),2)
        self.nConvs = _make_nConv(in_channels, out_channels, nb_Conv, activation)

    def forward(self, x, skip_x):
        out = self.up(x)
        x = torch.cat([out, skip_x], dim=1)  # dim label is the channel dimension
...
class UNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=9):
        ...
        n_channels : number of channels of the input.
                        By default img_npy, because we have RGB images
        n_labels : number of channels of the ouput.
                      By default img_npy (image labels + label for the background)
        '''
        super().__init__()
