...
def main():
    ...
    os.makedirs('xin_my_pro/gonggong/{}'.format(file_name),exist_ok=True)
    ...
    #save configuration
    with open('xin_my_pro/gonggong/{}/config.yml'.format(file_name), 'w') as f:
        yaml.dump(config, f)
    ...
    for epoch in range(config['epochs']):
        ...
        log.to_csv('xin_my_pro/gonggong/{}/log.csv'.format(file_name), index=False)

        trigger += 1

        if val_log['dice'] > best_dice:
            torch.save(model.state_dict(), 'xin_my_pro/gonggong/{}/bestmodel_{}_CosineAnnealingLR.pth'.format(file_name,config["lr"]))
